...
class CoreFinanceCustomerModal extends Model
{
  use HasFactory;
  ...
  {
    return $this->hasMany(CoreFinanceReceivableModal::class, 'customer_id');
  }
  ...
  {
    return $this->hasMany(CoreFinanceReceivablePaymentModal::class, 'customer_id');
  }
  ...
  /**
   * Scope a query to only include customers with overdue receivables.
   */
  public function scopeWithOverdueReceivables($query)
  {
  ...
  /**
   * Scope a query to only include customers exceeding credit limit.
   */
  public function scopeExceedingCreditLimit($query)
  {
    return $query->whereHas('receivables', function ($q) {
      $q->selectRaw('customer_id, SUM(remaining_amount) as total_outstanding')
        ->groupBy('customer_id')
        ->havingRaw('total_outstanding > finance_customers.credit_limit');
    });
  }
}
