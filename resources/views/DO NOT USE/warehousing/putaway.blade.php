...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Putaway Orders</h5>
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                            data-bs-target="#createPutawayOrderModal">
                            Create Putaway Order
                        </button>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    @foreach ($putawayOrders as $order)
                                        <tr>
                                            <td>
                                                <a href="javascript:void(0);" data-bs-toggle="modal"
                                                    data-bs-target="#viewPutawayOrderModal{{ $order->id }}">
                                                    #{{ $order->id }}
                                                </a>
                                            </td>
                                            ...
                                            <td>{{ $order->handler->name ?? 'Not Assigned' }}</td>
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        ...
                                                        @if ($order->status !== 'completed')
                                                            <a class="dropdown-item" href="javascript:void(0);"
                                                                onclick="assignHandler({{ $order->id }})">
                                                                <i class="ri-user-line me-1"></i> Assign Handler
                                                            </a>
                                                            <a class="dropdown-item" href="javascript:void(0);"
    ...
    <!-- Create Putaway Order Modal -->
    <div class="modal fade" id="createPutawayOrderModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Create Putaway Order</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('warehousing.putaway.store') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Receipt</label>
                                <select class="form-select" name="receipt_id" required>
                                    <option value="">Select Receipt</option>
                                    @foreach ($receipts as $receipt)
                                        <option value="{{ $receipt->id }}">#{{ $receipt->id }} -
                                            {{ $receipt->supplier->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
    ...
    @foreach ($putawayOrders as $order)
        <!-- View Putaway Order Modal -->
        <div class="modal fade" id="viewPutawayOrderModal{{ $order->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Putaway Order #{{ $order->id }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <p><strong>Handler:</strong> {{ $order->handler->name ?? 'Not Assigned' }}</p>
                                <p><strong>Created:</strong> {{ $order->created_at->format('M d, Y H:i') }}</p>
                                <p><strong>Completed:</strong>
                                    {{ $order->completed_at ? $order->completed_at->format('M d, Y H:i') : 'Not Completed' }}
                                </p>
                            </div>
                        </div>
                        <div class="table-responsive">
...
@section('page-script')
    ...
    `;
            document.getElementById('putawayItems').insertAdjacentHTML('beforeend', template);
            itemCount++;
        }
        ...
        function confirmDelete(orderId) {
            if (confirm('Are you sure you want to delete this putaway order?')) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = `/warehousing/putaway/${orderId}`;
                form.innerHTML = `
            @csrf
            @method('DELETE')
        `;
        ...
        function assignHandler(orderId) {
            // Implementation for assigning handler
        }
